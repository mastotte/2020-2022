...
//*								     *
//********************************************************************
float calculateSD_5(float data[], float p) {
...
void output_5(int best_round, int n, int best[n][n], int rounds, bool isTest, FILE* p){
  	char buf[50];
...
void penta(int players, int rounds, int TESTSIZE, bool isTest, FILE* p){
	srandom(SEED);
	int seed_input = 0;
	seed_input = random();
	...
	for(int test_count = players+1; test_count <= TESTSIZE; test_count++){
		...
		for(int j = 0; j < rounds; j++){
			...
			while(w_count >= 5){
				...
				if(C[r1]&&C[r2]&&C[r3]&&C[r4]&&C[r5]&&no_equals&&sit_pass){
					//printf(" | %d vs %d vs %d vs %d |",r1,r2,r3,r4);
					fprintf(outFile," %d vs %d vs %d vs %d vs %d -",r1+1,r2+1,r3+1,r4+1,r5+1);
					matches[r2][r1] += 1;
					matches[r1][r2] += 1;
		...
		if((zero_count <= players)||(players >= rounds)){
			stdev_new = calculateSD_5(data, (float) pow(players,2));
		}
	...
	fclose(outFile);
	output_5(best_round,players,best,rounds,isTest,p);
	fprintf(p,"\n");
