...
//*								     *
//********************************************************************
float calculateSD_2(float data[], float p) {
...
void output_2(int best_round, int n, int best[n][n],int rounds){
	char buf[50];
	printf("\nBest round = %d\n",best_round);
...
void doub(int players, int rounds){
	srandom(SEED);
	int seed_input = 0;
	seed_input = random();
	...
	for(int test_count = 1; test_count <= TESTSIZE; test_count++){
		...
		for(int j = 0; j < rounds; j++){
			...
			while(w_count >= 2){
				...
				if(C[r1]&&C[r2]&&no_equals&&sit_pass){
		...
		if((zero_count <= players)||(players >= rounds)){
			stdev_new = calculateSD_2(data, (float) pow(players,2));
		}
